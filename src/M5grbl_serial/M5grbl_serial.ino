...
void setup() {
  ...
  //_GRBL0.Gcode("$19=10");  // homing feed rate[mm/min]
  //_GRBL0.Gcode("$20=160");  // homing seek rate[mm/min]
  //_GRBL0.Gcode("$21=1");    // homing debounce[msec]
  ...
  if (M5.BtnC.wasPressed()) {
    if(fwork==1){
      if(fpause==0){
        fpause=1;
        M5.Lcd.setTextColor(RED, BLACK);
        _GRBL0.Gcode("G90G1Z0F800");
        _GRBL0.Gcode("G90G1X0Y0F800");
        _GRBL0.Gcode("finish");
        stop_LCD();
        fwork=0;
      }
...
void stop_LCD(){
  ...
  M5.Lcd.println("Stop Command");
  M5.Lcd.setTextSize(2);
  M5.Lcd.setCursor(60, 140);
  M5.Lcd.println("Operation stopped!");
  M5.Lcd.setCursor(15, 180);
  M5.Lcd.println("Please restart to resume");
}
...
void S_command(char data[1024]){
  ...
  if(data[1]=='1'){
    ...
    fileName.replace("S1:", "");
    M5.Lcd.setTextSize(3);
    //if(sizeof(fileName)>16){fileName[16]== '\0'; M5.Lcd.println("IN");}
  ...
  if(data[1]=='2'){
    ...
    totalCode=totalParts.toInt()*17+7;
  }
  if(data[1]=='3'){
    partsNum = data;
    partsNum.replace("S3:", "");
