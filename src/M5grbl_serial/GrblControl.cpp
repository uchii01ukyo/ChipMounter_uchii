...
//            data[i++] = Wire.read();
//        }
        for (i =0; i < 32; i++) data[i] = Wire.read();
        data[32] = '\0';
        Data += data;
//        Serial.print(i); Serial.print("{"); Serial.print(data);  Serial.println("}"); 
        if (data[31] == 0xff) break;
    }
    return Data;
    */
  /*
  // real all avaiable data in I2C bufer (by akita11, 2021/08/03)
    uint8_t i = 1;
    while (i > 0) {
      char data[33];
      i = 0;
